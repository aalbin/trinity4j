...
module.exports = function (settings, init_callback) {
	...
	var init = function (settings, init_callback) {
		...
		for (var key in schema.indices) {
			...
			trinity4j.ref[key] = (function (label) { return function (id) { return trinity4j.Reference(label, id); } })(key);
		...
		neo4j.runMultipleStatements({ queries: constraints, includeStats: true }, function (e, r) { init_callback(e.concat(errs), t4utils.concatNeoStats(r)); });
	};
	...
	this.add = function (dbSet, callback, mode, logging) {
		...
		trinity4j.add.getCypher(dbSet, function (cypher, params, errors) {
			...
			neo4j.runCypherQuery(cypher, params, function (neo_error, results) {
				...
				var stats = t4utils.concatNeoStats(results);
	...
	this.get.getCypher = function (getNodes, callback, mode, logging) {
		...
		if (tc.verify.is(getNodes, Object)) {
	...
	this.add.getCypher = function (dbSet, callback, mode, logging) {
		...
		var addBundle = function (r, fr, to) {
			...
			if (merge || [r, fr, to].all(function (n) { return n.commit_as_merge === true; })) {
		...
		iterateDbSet(dbSet, addReference, addReferenceByRelation, addNode, addNodeByRelation, addBundle, addRelation, function (errors) {
			// callback
	...
	this.set.getCypher = function (dbSet, callback, mode, logging) {
		...
		iterateDbSet(dbSet, handleReference, handleReferenceByRelation, handleNode, handleNodeByRelation, handleBundle, handleRelation, function (errors) {
			if (logging === 'verbose') {
				queries.forEach(function (q) {
					console.log('\ncypher:', '\n' + q.query);
					console.log(JSON.stringify(q.params, null, 2) + '\n');
	...
	this.drop = function (callback) {
		...
		neo4j.runMultipleStatements({ queries: constraints, includeStats: true }, function (e, r) {
			callback(e, t4utils.concatNeoStats(r));
		});
	}
	...
	var Reference = function (label, identifier) {
		var ref = this,
	...
	this.Reference = function (label, identifier) {
		var index = getIndex(label);
		if (index) { // .id && identifier) {	//temporarily removing requirement for Reference to include an identifier - can be added later by chaining 
			return new Reference(label, identifier);
		}
	}
	...
	var nodeCreate = function (n) { return "(" + n.__key + ":" + n.label + " {" + n.__key + "})"; }
	...
	var nodeRef = function (n) { return "(" + n.__key + ")"; }
	var nodeMerge = function (n) { return '(' + n.__key + ':' + n.label + ' {' + getIndex(n.label).id + ':' + JSON.stringify(n.identifier) + '})' + onCreateSet(n); }
	var referenceMatchByRelation = function (n) {
		...
		return (dir === '<-' ? matchThat : matchThis) + "-[" + n.__key + n.identifier.vertex.__key + (n.identifier.type === type_all ? "" : ":" + n.identifier.type) + "]->" + (dir === '->' ? matchThat : matchThis);
	}
	var relationCreate = function (r) { return "[" + r.__key + (r.type === type_all ? "" : ":" + r.type) + (r.data ? " {" + r.__key + "}" : "") + "]"; }
	var relationRef = function (r) { return "[" + r.__key + (r.type === type_all ? "" : ":" + r.type) + "]"; }
	...
	var iterateDbSet = function (dbSet, refHandler, refByRelHandler, nodeHandler, nodeByRelHandler, bundleHandler, relHandler, callback) {
		// dbSet instanceof DbSet
		// refHandler(n);
		// refByRelHandler(n);
		// nodeHandler(n);
